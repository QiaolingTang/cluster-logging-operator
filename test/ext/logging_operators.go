...
var _ = g.Describe("[sig-openshift-logging] Logging NonPreRelease vector-loki Upgrade testing loki-operator", func() {
	...
	g.It("Author:qitang-Longduration-CPaasrunOnly-Critical-53407-Upgrade with Vector as collector - minor version.[Serial][Slow]", func() {
		g.Skip("Skip for logging 6.3 is not released!")
		var targetchannel = "stable-6.3"
		var oh OperatorHub
	...
	g.It("Author:qitang-Longduration-CPaasrunOnly-Critical-53404-Upgrade with Vector as collector - major version LokiStack [Serial][Slow]", func() {
		// to add logging 6.2, create a new catalog source with image: quay.io/openshift-qe-optional-operators/aosqe-index
		catsrcTemplate := compat_otp.FixturePath("testdata", "logging", "subscription", "catsrc.yaml")
		...
		// for 6.3, test upgrade from 6.2 to 6.3
		preSource := CatalogSourceObjects{"stable-6.2", catsrc.name, catsrc.namespace}
		...
		var source = CatalogSourceObjects{"stable-6.3", "qe-app-registry", "openshift-marketplace"}
		//change channel, and wait for the new operators to be ready
