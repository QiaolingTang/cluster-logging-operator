...
import (
	...
	_ "github.com/openshift/cluster-logging-operator-tests-extension/test/e2e/testdata"

	// Import test packages from this module
	_ "github.com/openshift/cluster-logging-operator-tests-extension/test/e2e"
)
...
func main() {
	...
	ext := e.NewExtension("openshift", "payload", "cluster-logging-operator")
	...
	root := &cobra.Command{
		Long: "Cluster Logging Operator Tests",
	}
...
func registerSuites(ext *e.Extension) {
	suites := []e.Suite{
		{
			Name: "cluster-logging-operator/conformance/parallel",
			Parents: []string{
		...
		{
			Name: "cluster-logging-operator/conformance/serial",
			Parents: []string{
		...
		{
			Name:        "cluster-logging-operator/disruptive",
		...
		{
			Name:        "cluster-logging-operator/non-disruptive",
		...
		{
			Name:        "cluster-logging-operator/all",
			Description: "All cluster-logging-operator tests",
