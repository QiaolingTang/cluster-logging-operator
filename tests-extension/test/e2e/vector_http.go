...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc             = compat_otp.NewCLI("logfwdhttp", compat_otp.KubeConfigPath())
	...
	g.Context("Log Forward to Http", func() {
		...
		g.It("Author:anli-CPaasrunOnly-WRS-Medium-61253-V-ICA.03-vector forward logs to fluentdserver over http - mtls", func() {
...
ca_file = "/var/run/ocp-collector/secrets/to-fluentd-60933/ca-bundle.crt"`
		...
		g.It("Author:anli-CPaasrunOnly-WRS-Critical-65131-V-ICA.02 mCLF Inputs.receiver.http over http with default values", func() {
			clfNS := oc.Namespace()
			fluentdNS := clfNS
