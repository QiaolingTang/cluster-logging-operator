...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc             = compat_otp.NewCLI("vector-es", compat_otp.KubeConfigPath())
	...
	g.Context("Log Forward to User-Managed-ES", func() {
		...
		g.It("Author:ikanse-CPaasrunOnly-High-55396-AlertRule CollectorNodeDown testing", func() {
			g.By("Create external Elasticsearch instance")
			esProj := oc.Namespace()
			ees := externalES{
				namespace:  esProj,
...
retry_max_duration_secs = 30`,
		...
		g.It("Author:anli-CPaasrunOnly-High-74322-CLO expose Metrics,AlertRule and Telemetry", func() {
			g.By("Create external Elasticsearch instance")
			esProj := oc.Namespace()
			ees := externalES{
				namespace:  esProj,
		...
		g.It("Author:qitang-CPaasrunOnly-Medium-86567-Add custom headers when forwarding to Elasticsearch output.", func() {
			g.By("Create external Elasticsearch instance")
			esProj := oc.Namespace()
			ees := externalES{
				namespace:  esProj,
			...
			expectedConfigs := []string{
				`[sinks.output_es_created_by_user.request]
headers = {"Accept"="application/json","test"="logging"}`,
			}
			result, err := checkCollectorConfiguration(oc, clf.namespace, clf.name+"-config", expectedConfigs...)
			o.Expect(err).NotTo(o.HaveOccurred())
			o.Expect(result).Should(o.BeTrue())
