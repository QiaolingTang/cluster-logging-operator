...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	defer g.GinkgoRecover()

	var (
		oc             = compat_otp.NewCLI("vector-loki", compat_otp.KubeConfigPath())
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc                    = compat_otp.NewCLI("vector-lokistack", compat_otp.KubeConfigPath())
	...
	g.Context("Log Forward to LokiStack", func() {
		...
		g.It("Author:qitang-CPaasrunOnly-ConnectedOnly-Medium-48646-Medium-49486-Deploy LokiStack under different namespace and Vector Forward logs to LokiStack using CLF with gateway[Serial]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-49486", //notsecret
				storageClass:  sc,
		...
		g.It("Author:kbharti-CPaasrunOnly-ConnectedOnly-Medium-54663-Medium-48628-unique cluster identifier in all type of the log record and Expose LokiStack metrics to Prometheus[Serial]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-secret-54663", //notsecret
				storageClass:  sc,
		...
		g.It("Author:kbharti-CPaasrunOnly-ConnectedOnly-High-57063-Forward app logs to LokiStack with namespace selectors (vector)[Serial]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-secret-57063", //notsecret
				storageClass:  sc,
		...
		g.It("Author:qitang-CPaasrunOnly-High-74945-New filter detectMultilineException test[Serial][Slow]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-secret-74945", //notsecret
				storageClass:  sc,
		...
		g.It("Author:qitang-CPaasrunOnly-ConnectedOnly-Medium-71144-Collect or exclude infrastructure logs[Serial][Slow]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-secret-71144", //notsecret
				storageClass:  sc,
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc             = compat_otp.NewCLI("vector-loki-ext", compat_otp.KubeConfigPath())
		loggingBaseDir string
	)
...
max_size = 268435488`,
		...
		g.It("Author:ikanse-CPaasrunOnly-High-62975-Collector connects to the remote output using the cipher defined in the tlsSecurityProfile [Slow][Disruptive]", func() {
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc                    = compat_otp.NewCLI("lokistack-tlssecurity", compat_otp.KubeConfigPath())
		loggingBaseDir, s, sc string
	)
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc                = compat_otp.NewCLI("loki-log-alerts-vector", compat_otp.KubeConfigPath())
	...
	g.Context("LokiStack Log Alerts testing loki-operator", func() {
		...
		g.It("Author:kbharti-CPaasrunOnly-High-52779-High-55393-Validate alert and recording rules in LokiRuler configmap and Rules API(cluster-admin)[Serial]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-52779", //notsecret
				storageClass:  sc,
		...
		g.It("Author:kbharti-CPaasrunOnly-Critical-55415-Validate AlertManager support for cluster-monitoring is decoupled from User-workload monitoring[Serial]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-55415", //notsecret
				storageClass:  sc,
		...
		g.It("Author:kbharti-CPaasrunOnly-Medium-61435-Validate AlertManager support for User-workload monitoring[Serial]", func() {
			...
			ls := lokiStack{
				...
				tSize:         "1x.demo",
				storageType:   s,
				storageSecret: "storage-61435", //notsecret
				storageClass:  sc,
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease Flow control testing", func() {
	defer g.GinkgoRecover()

	var (
		oc                                 = compat_otp.NewCLI("logging-flow-control", compat_otp.KubeConfigPath())
	...
	g.It("Author:qitang-CPaasrunOnly-Medium-76114-Controlling log flow rates per container from selected containers by containerLimit LokiStack[Serial][Slow]", func() {
		...
		ls := lokiStack{
			...
			tSize:         "1x.demo",
			storageType:   s,
			storageSecret: "storage-secret-76114", //notsecret
			storageClass:  sc,
	...
	g.It("Author:qitang-CPaasrunOnly-Medium-76115-Controlling the flow rate per destination to selected outputs LokiStack [Serial][Slow]", func() {
		...
		ls := lokiStack{
			...
			tSize:         "1x.demo",
			storageType:   s,
			storageSecret: "storage-secret-76115", //notsecret
			storageClass:  sc,
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease Audit Policy Testing", func() {
	defer g.GinkgoRecover()

	var (
		oc                    = compat_otp.NewCLI("logging-audit-policy", compat_otp.KubeConfigPath())
		loggingBaseDir, s, sc string
	)
	...
	g.It("Author:qitang-CPaasrunOnly-Critical-75841-Filter audit logs and forward to log store LokiStack[Serial]", func() {
		...
		ls := lokiStack{
			...
			tSize:         "1x.demo",
			storageType:   s,
			storageSecret: "storage-75841", //notsecret
			storageClass:  sc,
	...
	g.It("Author:qitang-CPaasrunOnly-High-67421-Separate policies can be applied on separate pipelines LokiStack.[Serial]", func() {
		...
		ls := lokiStack{
			...
			tSize:         "1x.demo",
			storageType:   s,
			storageSecret: "storage-67421", //notsecret
			storageClass:  sc,
	...
	g.It("Author:qitang-CPaasrunOnly-Medium-68318-Multiple policies can be applied to one pipeline LokiStack.[Serial]", func() {
		...
		ls := lokiStack{
			...
			tSize:         "1x.demo",
			storageType:   s,
			storageSecret: "storage-68318", //notsecret
			storageClass:  sc,
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease Loki Fine grained logs access testing loki-operator", func() {
	defer g.GinkgoRecover()

	var (
		oc                    = compat_otp.NewCLI("loki-logs-access", compat_otp.KubeConfigPath())
		loggingBaseDir, s, sc string
	)
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease LokiStack - Efficient OTEL Support", func() {
	defer g.GinkgoRecover()

	var (
		oc                    = compat_otp.NewCLI("loki-otel-support", compat_otp.KubeConfigPath())
		loggingBaseDir, s, sc string
	)
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease - LokiStack with tenant level labelkeys", func() {
	defer g.GinkgoRecover()

	var (
		oc                    = compat_otp.NewCLI("lokistack-labelkeys", compat_otp.KubeConfigPath())
		loggingBaseDir, s, sc string
	)
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease - LokiStack with OTLP support", func() {
	defer g.GinkgoRecover()

	var (
		oc                    = compat_otp.NewCLI("lokistack-otlp-flow", compat_otp.KubeConfigPath())
		loggingBaseDir, s, sc string
	)
...
resource_attributes:
  attributes_config:
	...
	g.It("Author:kbharti-CPaasrunOnly-High-81978-Verify non-admin users can query with Otel semantic conventions for ViaQ dataModel[Serial]", func() {
	...
	g.It("Author:kbharti-CPaasrunOnly-High-82009-Verify non-admin users can query with Otel semantic conventions for Otel dataModel[Serial]", func() {
	...
	g.It("Author:kbharti-CPaasrunOnly-High-82010-Verify Loki AlertingRule creation with Otel attribute k8s_namespace_name for namespace matcher[Serial]", func() {
		jsonLogFile := filepath.Join(loggingBaseDir, "generatelog", "container_json_log_template.json")
		appProj := oc.Namespace()
		oc.AsAdmin().WithoutNamespace().Run("label").Args("namespace", appProj, "openshift.io/cluster-monitoring=true").Execute()
		err := oc.WithoutNamespace().Run("new-app").Args("-n", appProj, "-f", jsonLogFile).Execute()
		...
		ls := lokiStack{
			...
			tSize:         "1x.demo",
			storageType:   s,
			storageSecret: "storage-82010", //notsecret
			storageClass:  sc,
	...
	g.It("Author:kbharti-CPaasrunOnly-High-83593-Verify Loki AlertingRules can fire when dataModel is Otel[Serial]", func() {
	...
	g.It("Author:qitang-CPaasrunOnly-High-85395-Validate the default maximum request size is set to 3Mi when forwarding to LokiStack with Otel dataModule.[Serial]", func() {
		jsonLogFile := filepath.Join(loggingBaseDir, "generatelog", "container_json_log_template.json")
		appProj := oc.Namespace()
		oc.AsAdmin().WithoutNamespace().Run("label").Args("namespace", appProj, "openshift.io/cluster-monitoring=true").Execute()
		err := oc.WithoutNamespace().Run("new-app").Args("-n", appProj, "-f", jsonLogFile).Execute()
...
type = "opentelemetry"`,
			`[sinks.output_lokistack_application.protocol.batch]
max_bytes = 3145728`,
			`[sinks.output_lokistack_audit]
type = "opentelemetry"`,
			`[sinks.output_lokistack_audit.protocol.batch]
max_bytes = 3145728`,
			`[sinks.output_lokistack_infrastructure.protocol.batch]
max_bytes = 3145728`,
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease - LokiStack Virtual Host Style of S3 URL", func() {
	defer g.GinkgoRecover()

	var (
		oc                 = compat_otp.NewCLI("lokistack-vhs-flow", compat_otp.KubeConfigPath())
	...
	g.It("Author:kbharti-CPaasrunOnly-Medium-82043-Configure virtual-host-style of S3 URL for minio buckets used on LokiStack object storage[Serial]", func() {
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease - LokiStack Network Policies", func() {
	defer g.GinkgoRecover()
	...
	g.It("Author:kbharti-CPaasrunBoth-High-85643-High-85644-Enable and Disable network policies on an existing instance of LokiStack[Serial]", func() {
	...
	g.It("Author:kbharti-CPaasrunBoth-High-85645-Verify Loki Operator reconciles if network policies are deleted manually[Serial]", func() {
