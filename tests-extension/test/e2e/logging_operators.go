...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease vector-loki Upgrade testing loki-operator", func() {
	defer g.GinkgoRecover()
	var (
		oc             = compat_otp.NewCLI("logging-loki-upgrade", compat_otp.KubeConfigPath())
	...
	*/
	...
	g.It("Author:qitang-Longduration-CPaasrunOnly-Critical-53407-Upgrade with Vector as collector - minor version.[Serial][Slow]", func() {
		g.Skip("skip the case for logging 6.5 is not released")
		var targetchannel = "stable-6.5"
	...
	g.It("Author:qitang-Longduration-CPaasrunOnly-Critical-53404-Upgrade with Vector as collector - major version LokiStack [Serial][Slow]", func() {
		// for 6.5, test upgrade from 6.4 to 6.5
		preSource := CatalogSourceObjects{"stable-6.4", "redhat-operators", "openshift-marketplace"}
		...
		version := "6.5"
		g.By("upgrade CLO&LO to stable-6.5")
		err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("-n", preCLO.Namespace, "sub/"+preCLO.PackageName, "-p", "{\"spec\": {\"channel\": \"stable-6.5\", \"source\": \"cluster-logging-operator-registry\", \"sourceNamespace\": \"openshift-marketplace\"}}", "--type=merge").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
		err = oc.AsAdmin().WithoutNamespace().Run("patch").Args("-n", preLO.Namespace, "sub/"+preLO.PackageName, "-p", "{\"spec\": {\"channel\": \"stable-6.5\", \"source\": \"loki-operator-registry\", \"sourceNamespace\": \"openshift-marketplace\"}}", "--type=merge").Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease OperatorDeployment", func() {
	defer g.GinkgoRecover()
	var (
		oc             = compat_otp.NewCLI("logging-operators", compat_otp.KubeConfigPath())
		loggingBaseDir string
	)
	...
	g.It("Author:anli-CPaasrunOnly-Low-65518-deploy cluster-logging-operator after Datadog-Agent is deployed [Serial]", func() {
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease multi-mode testing", func() {
	defer g.GinkgoRecover()
	var (
		oc             = compat_otp.NewCLI("logging-multiple-mode", compat_otp.KubeConfigPath())
		loggingBaseDir string
	)
	...
	g.It("Author:qitang-CPaasrunOnly-Medium-64147-Deploy LFME as an independent pod[Serial]", func() {
		...
		ls := lokiStack{
			...
			tSize:         "1x.demo",
			storageType:   getStorageType(oc),
			storageSecret: "storage-64147", //notsecret
			storageClass:  sc,
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease rapidast scan", func() {
	defer g.GinkgoRecover()
	var (
		oc             = compat_otp.NewCLI("logging-dast", compat_otp.KubeConfigPath())
		loggingBaseDir string
	)
	g.BeforeEach(func() {
		loggingBaseDir = testdata.FixturePath("logging")
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease must-gather", func() {
	defer g.GinkgoRecover()

	var (
		oc                    = compat_otp.NewCLI("logging-must-gather", compat_otp.KubeConfigPath())
...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease NetworkPolicy", func() {
	defer g.GinkgoRecover()
	var (
		oc                          = compat_otp.NewCLI("logging-np", compat_otp.KubeConfigPath())
	...
	g.It("Author:qitang-CPaasrunOnly-High-84892-High-84897-NetworkPolicy for LFME and ClusterLogForwarder LokiStack output.[Serial]", func() {
		sc, _ := getStorageClassName(oc)
		if len(sc) == 0 {
			g.Skip("The cluster doesn't have a storage class for this test!")
		}
	...
	g.It("Author:qitang-CPaasrunOnly-High-85402-NetworkPolicy for ClusterLogForwarder OTLP output.", func() {
		var (
			expectedCSV       string
	...
	// author qitang@redhat.com
	g.It("Author:qitang-CPaasrunOnly-High-85456-NetworkPolicy for ClusterLogForwarder CloudWatch output.", func() {
	...
	// author qitang@redhat.com
	g.It("Author:qitang-CPaasrunOnly-High-85744-NetworkPolicy for ClusterLogForwarder s3 output.", func() {
		platform := compat_otp.CheckPlatform(oc)
		if platform != "aws" {
			g.Skip("Skip for the platform is not AWS.")
		}
	...
	//author qitang@redhat.com
	g.It("Author:qitang-CPaasrunOnly-ConnectedOnly-High-85410-NetworkPolicy for ClusterLogForwarder GCL output.", func() {
		platform := compat_otp.CheckPlatform(oc)
		if platform != "gcp" {
	...
	// author qitang@redhat.com
	g.It("Author:qitang-CPaasrunOnly-High-85411-NetworkPolicy for ClusterLogForwarder elasticsearch output.", func() {
	...
	// author qitang@redhat.com
	g.It("Author:qitang-CPaasrunOnly-High-85466-High-85490-High-85465-NetworkPolicy for ClusterLogForwarder inputs.receiver.http and http, splunk output[Serial]", func() {
		nodes, err := oc.AdminKubeClient().CoreV1().Nodes().List(context.Background(), metav1.ListOptions{LabelSelector: "kubernetes.io/os=linux,kubernetes.io/arch=amd64"})
		if err != nil || len(nodes.Items) == 0 {
			g.Skip("Skip for the cluster doesn't have amd64 node")
		}
	...
	g.It("Author:qitang-CPaasrunOnly-High-85469-High-85472-NetworkPolicy for ClusterLogForwarder inputs.receiver.syslog and syslog output.[Serial][Slow]", func() {
		sc, _ := getStorageClassName(oc)
		if len(sc) == 0 {
			g.Skip("The cluster doesn't have a storage class for this test!")
		}
	...
	//author qitang@redhat.com
	g.It("Author:qitang-CPaasrunOnly-ConnectedOnly-High-85496-NetworkPolicy for ClusterLogForwarder azureMonitor output.", func() {
		platform := compat_otp.CheckPlatform(oc)
		if platform != "azure" {
	...
	g.It("Author:qitang-CPaasrunOnly-High-85491-NetworkPolicy for ClusterLogForwarder Kafka output.", func() {
		g.By("Create log producer")
		appProj := oc.Namespace()
		jsonLogFile := filepath.Join(loggingBaseDir, "generatelog", "container_json_log_template.json")
		err := oc.WithoutNamespace().Run("new-app").Args("-n", appProj, "-f", jsonLogFile).Execute()
