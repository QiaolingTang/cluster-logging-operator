...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease Otlp output testing", func() {
	defer g.GinkgoRecover()
	var (
		oc             = compat_otp.NewCLI("vector-otlp", compat_otp.KubeConfigPath())
	...
	g.It("Author:qitang-CPaasrunOnly-Critical-68961-Medium-85651-Forward logs to OTEL collector and use opentelemetry sink when forwarding to OTLP output", func() {
		var (
			expectedCSV       string
		...
		expectedConfigs := []string{
			"compression = \"gzip\"",
			`[sinks.output_otlp.protocol.batch]
max_bytes = 10000000`,
