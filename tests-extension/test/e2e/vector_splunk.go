...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc             = compat_otp.NewCLI("vector-splunk", compat_otp.KubeConfigPath())
	...
	g.Context("Splunk Custom Tenant", func() {
		...
		g.It("Author:anli-CPaasrunOnly-High-81855-Vector forward logs to Splunk - customized source", func() {
			oc.SetupProject()
			splunkProject := oc.Namespace()
			sp := splunkPodServer{
				namespace: splunkProject,
