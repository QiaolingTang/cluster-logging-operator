...
var _ = g.Describe("[OTP][sig-openshift-logging] Logging NonPreRelease", func() {
	...
	var (
		oc             = compat_otp.NewCLI("vector-kafka", compat_otp.KubeConfigPath())
	...
	g.Context("Log Forward to Kafka", func() {
		...
		g.It("Author:anli-CPaasrunOnly-WRS-Critical-68312-V-ICA.02-V-ICA.03-Forward to Kafka using SSL-SASL_SCRAM auth", func() {
			amqNS := oc.Namespace()
		...
		// author qitang@redhat.com
		g.It("Author:qitang-CPaasrunOnly-Medium-48141-Medium-85766-Vector Forward logs to different Kafka brokers and test NetworkPolicy for Kafka multi-brokers.[Slow]", func() {
			nodes, err := oc.AdminKubeClient().CoreV1().Nodes().List(context.Background(), metav1.ListOptions{LabelSelector: "kubernetes.io/os=linux,kubernetes.io/arch=amd64"})
			if err != nil || len(nodes.Items) == 0 {
				g.Skip("Skip for the cluster doesn't have amd64 node")
			}
